...
const { optionalAuth, optionalAuthOrApiKey, verifyTokenOrApiKey } = require('../middleware/auth');
...
// Compile sketch
router.post('/compile', optionalAuthOrApiKey, upload.fields([
...
// Install library
router.post('/install-lib', optionalAuthOrApiKey, upload.single('libZip'), async (req, res) => {
  try {
    const fd = new FormData();
