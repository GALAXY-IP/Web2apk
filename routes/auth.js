...
router.post('/google-firebase', async (req, res) => {
  try {
    const { uid, email, displayName, photoURL, resellerRef } = req.body;
    if (!uid || !email) return res.json({ success: false, error: 'Data Firebase tidak lengkap' });

    // Cari user berdasarkan firebaseUid atau email
    let user = db.findUser({ firebaseUid: uid }) || db.findUser({ email });

    if (!user) {
      // Buat user baru dari Google login
      const username = (displayName || email.split('@')[0]).replace(/\s+/g, '_').toLowerCase().substring(0, 20);
      let finalUsername = username;
      let counter = 1;
      while (db.findUser({ username: finalUsername })) {
      ...
      user = db.createUser({
        ...
        resellerRef: resellerRef || null,
        password: null
      });
    } else {
      // Update firebaseUid dan photoURL jika perlu
      const updates = {};
      if (!user.firebaseUid) updates.firebaseUid = uid;
      if (photoURL && !user.photoURL) updates.photoURL = photoURL;
      if (Object.keys(updates).length > 0) user = db.updateUser(user.id, updates) || user;
    }
    ...
    console.error('Google Firebase login error:', e);
    res.json({ success: false, error: 'Login Google gagal' });
  }
});
