...
// Middleware yang support BOTH JWT token AND API Key
// Sehingga semua endpoint bisa diakses via API key
function verifyTokenOrApiKey(req, res, next) {
  // Coba API Key dulu (X-API-Key header)
  const apiKey = req.headers['x-api-key'];
  if (apiKey) {
    const user = db.findUser({ apiKey });
    if (!user) return res.json({ success: false, error: 'API Key tidak valid' });
    if (user.status === 'banned') return res.json({ success: false, error: 'Akun diblokir' });
    req.user = db.checkAndExpireRole(user);
    req.isApiKeyAuth = true;
    return next();
  }
  // Fallback ke JWT token
  return verifyToken(req, res, next);
}

// Optional auth yang juga support API key
function optionalAuthOrApiKey(req, res, next) {
  const apiKey = req.headers['x-api-key'];
  if (apiKey) {
    const user = db.findUser({ apiKey });
...
module.exports = { verifyToken, optionalAuth, verifyAdmin, generateToken, generateAdminToken, getAdminCredentials, verifyTokenOrApiKey, optionalAuthOrApiKey };
